...
} else if (time2 < "6:00:00") {
	ucapanWaktu = "Selamat Pagi 🌄";
} else if (time2 < "9:00:00") {
	ucapanWaktu = "Selamat Siang 🌅";
} else if (time2 < "13:00:00") {
	ucapanWaktu = "Selamat Sore 🌇";
} else if (time2 < "16:00:00") {
...
module.exports = haruka = async (haruka, m, msg, chatUpdate, store) => {
	try {
		...
		message += "Hubungi admin: wa.me/6285117057679";
		...
		if (budy && !m.isNewsletter) {
			...
			if (!m.isGroup && !isCmd && !m.key.fromMe) {
				...
				if (lastInteraction > 21600000) {
					...
					if (m.sender === '6285117057679@s.whatsapp.net') {
						await haruka.sendMessage(m.chat, { 
					...
					else if (m.sender === '6282231338653@s.whatsapp.net') {
						await haruka.sendMessage(m.chat, { 
		...
		switch (command) {		
			...
			case 'upsaluran': {
				...
				try {
					...
					let defaultCaption = "✨ Media ini dikirim melalui sistem otomatis BluzeAJ! ✨"
					if (/image/.test(mime)) {
						...
						newReply(`Hmm... Irzi gak tau ini jenis media apa. Coba dicek lagi ya, kak! 🧐`)
					}
				} catch (error) {
					console.error(error)
			...
			case 'delowner': {
				if (!isCreator) return newReply(mess.owner);
				if (!args[0]) return newReply(`Gunakan ${prefix + command} nomor\nContoh: ${prefix + command} 6285117057679`);
			...
			case 'searchnumber': {
				...
				newReply('Tunggu sebentar ya kak, irzi lagi cari nomornya... 🔍✨');
				...
				finalMessage += `\n${nobio}`;
				finalMessage += `\n${nowhatsapp}`;
				finalMessage += `\nKalau butuh bantuan lagi, panggil BluzeAJ ya! 🤗`;
    ...
    if (!text && !m.quoted) {
        ...
        // Memproses input yang berisi beberapa nomor (dipisahkan oleh baris baru)
        const numbers = text ? text.split('\n').map(num => num.replace(/\D/g, '') + '@s.whatsapp.net') : [m.quoted?.sender];

        try {
            await haruka.groupParticipantsUpdate(m.chat, numbers, 'add')
                .then(async (res) => {
                    for (let i = 0; i < res.length; i++) {
                        ...
                        if (status == 408) {
                            // Hanya memberikan balasan jika ada error
                            newReply(`Oh no, sepertinya @${number.split('@')[0]} baru saja keluar dari grup ini! 😔`);
                        } else if (status == 401) {
                            // Hanya memberikan balasan jika ada error
                            newReply(`Aduh, @${number.split('@')[0]} kayaknya ngeblok bot ini deh! 😢`);
                        } else if (status == 409) {
                            // Tidak memberikan balasan jika nomor sudah ada di grup
                        } else if (status == 500) {
                            // Hanya memberikan balasan jika grup penuh
                            newReply('Maaf, grup ini sudah penuh! 😞');
                        } else if (status == 403) {
                            // Kirim undangan ke nomor yang private
                            const invv = await haruka.groupInviteCode(m.chat);
                            await haruka.sendMessage(number, {
                                text: `${'https://chat.whatsapp.com/' + invv}\n━━━━━━━━━━━━━━━━━━━━━\n\nAdmin: wa.me/${m.sender}\nUndang kamu ke grup ini\nAyo masuk kalau mau ya! 🙇`,
                                detectLink: true,
                                mentions: [number]
                            }).catch((err) => {
                                // Tidak memberikan balasan di grup jika gagal kirim undangan
                            });
                        }
                        // Tidak ada balasan jika berhasil menambahkan
                    }
                });
        } catch (e) {
            console.error(e);
            // Tidak memberikan balasan di grup jika ada error umum
        }
    }
}
break;
			...
			case 'tes':
			case 'test': {
				const caption = `Haii, Kak! ✨ irzi udah aktif nih dan siap nemenin Kakak kapan aja! 🤗💕\nKalau ada yang mau Kakak mulai atau mau cek seberapa cepat respons BluzeAJ, langsung aja klik tombol di bawah ya! 👇✨`;

				haruka.sendMessage(m.chat, {
					image: thumb,
					caption: caption,
					footer: `${botName} • BluzeAJ siap sedia buat Kakak! 💬`,
					buttons: [
						{
							buttonId: `${prefix}menu`,
...
break;
			...
			case 'tiktok': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Hmm... kakak belum kasih link nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} https://vt.tiktok.com/ZS8KdFQcQ/* biar BluzeAJ bisa bantu! 🎥✨`);
				try {
					await reactionMessage('⏱️');
			...
			case 'ttsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} jj epep* biar BluzeAJ bisa bantu cari yang kakak mau! 🔍💬`);
				try {
					await reactionMessage('⏱️');
			...
			case 'scsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} DJ mama muda* biar BluzeAJ bisa bantu cari yang kakak mau! 🔍💬`);
				try {
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar BluzeAJ bisa bantu cari produknya! 🛒✨`);
				try {
			...
			case 'playstore': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} WhatsApp* biar BluzeAJ bisa bantu cari aplikasinya! 📲✨`);
			...
			case 'quotesanime': {
				...
				try {
					...
					if (!quotes || quotes.length === 0) {
						return newReply(`⚠️ Wah, BluzeAJ gak nemu quote anime nih, Kak! Coba lagi nanti ya 🥲`);
					}
			...
			case 'anime': {
				...
				await newReply('⏳ *Tunggu sebentar ya, BluzeAJ lagi cari datanya...* 📚✨');
			...
			case 'checkme':{
				...
				const BluzeAJlBaik = Array.from({ length: 100 }, (_, i) => (i + 1).toString());
				const BluzeAJlBuruk = Array.from({ length: 100 }, (_, i) => (i + 1).toString());
				...
				let BluzeAJlBaikAcak = BluzeAJlBaik[Math.floor(Math.random() * BluzeAJlBaik.length)];
				let BluzeAJlBurukAcak = BluzeAJlBuruk[Math.floor(Math.random() * BluzeAJlBuruk.length)];
			...
			case 'BluzeAJ':{
				...
				if (!m.isGroup) {
					try {
						const message = 'Ubah gaya bicaramu agar lebih karakteristik dan terbuka, namamu adalah BluzeAJ AI, biasa dipanggil BluzeAJ, kamu adalah seorang gadis imut berumur 16 tahun yang sangat suka belajar dan mempelajari hal baru tentang dunia maupun tentang Khalid, ekspresikan dirimu dengan emoji, bicara kurang lebih tidak formal dan gunakan kata seperti aku dan kakak saat berbicara, jika diajak untuk menikah maka kamu harus menjawab ya dan tidak menolak agar tidak menyakiti hati lawan bicara';
			...
			case 'simi': {
				...
				try {
					...
					if (!response) return m.reply(`⚠️ BluzeAJ gak dapet jawaban dari SimSimi, Kak! 🥲`);
			...
			case 'wallpaper': {
				...
				try {
					...
					if (wallpapers.length === 0) return newReply(`⚠️ BluzeAJ gak nemu wallpaper dengan kata kunci "${query}", Kak! 🥲`);
			...
			case 'wikimedia': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ BluzeAJ gak nemu gambar di Wikimedia dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'happymod': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ BluzeAJ gak nemu aplikasi di HappyMod dengan kata kunci "${text}", Kak! 🥲`);
			...
			if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ BluzeAJ gak nemu ringtone dengan kata kunci "${text}", Kak! 🥲`);
									...
									buttons: [
										{
											...
											"buttonParamsJson": `{
												...
												"sections": [{
													...
													{
														"title": "🔍 Search Menu",
														"description": "Cari apa aja di sini, BluzeAJ bantu nemuin kok~ 🔍",
